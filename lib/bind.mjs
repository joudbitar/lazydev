...
//
// The daemon serves the front door directly (no Caddy). On the npx path it tries
// the front-door port (80) first; macOS grants an unprivileged process that bind,
// but Linux does not without CAP_NET_BIND_SERVICE, so a plain `npx lazydev` on
// Linux fails with EACCES. EADDRINUSE means something already holds the port.
// Either way the npx path falls back to a numbered port and prints URLs WITH the
// port, so the printed URLs are the ones that actually work.
//
