...
The PR I'd merge first is the Linux installer: a systemd unit with `CAP_NET_BIND_SERVICE` so Linux gets the portless URLs too. The daemon already passes its tests on Linux; only `install.sh` is macOS.
