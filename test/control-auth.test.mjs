...
test('start-error page redacts the log tail for unauthorized callers, shows it for authorized', async (t) => {
  ...
  // Unauthorized proxy request (no token): the dev server never opens its port,
  // so ensureUp times out (startTimeoutMs=300) and the 502 page is served WITHOUT
  // the log tail. We key on the fixed marker string the authorized page uses.
  const unauth = await httpReq(port, {
    method: 'GET',
    pathname: '/',
    headers: { host: 'fake.localhost' },
  });
  assert.equal(unauth.status, 502, `failed start should be 502 (got ${unauth.status})`);
  ...
  // Authorized caller (valid token): same request path, but the tail is included.
  const token = ensureControlToken();
  const auth = await httpReq(port, {
    method: 'GET',
    pathname: '/',
    headers: { host: 'fake.localhost', 'x-lazydev-token': token },
  });
  assert.equal(auth.status, 502, `failed start should be 502 (got ${auth.status})`);
