...
function printSummary(projects, servePort) {
  ...
  process.stdout.write(`to keep it running in the background: run ./install.sh (persistent daemon, same state dir).\n\n`);
}
...
async function main() {
  ...
  if (arg === 'install') {
    process.stdout.write(
      ...
      'it uses the same state directory as this npx run, so your registry carries over.\n'
    );
    return 0;
  }
